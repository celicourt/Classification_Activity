...
# python3 predict_video.py --model model/activity.model --label-bin model/lb.pickle --input example_clips/cyclone_movie.mp4 --output output/cyclon_movie_output.avi --size 10
...
#	print('Old threshold = ', th)
	...
	if (preds[prediction]) < th:
		text = "Activity : {} - {:.2f}%".format((ok), 100 - (maxprob * 100))
		cv2.putText(output, text, (35, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (0, 255, 0), 5)
	else:
		fi_label = np.append(fi_label, label)
		text1 = "Activity : {} - {:.2f}%".format((label), maxprob * 100)
		cv2.putText(output, text1, (35, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (0, 255, 0), 3) 
		text = "Frames count : {0} ".format((framecount))
		cv2.putText(output, text, (00, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2) 
