# USAGE
# python3 train.py --dataset data --model model/activity.model --label-bin model/lb.pickle --epochs 80
...
LABELS = set(["Cought", "Fight", "Use Phone"])
...
H = model.fit_generator(
	trainAug.flow(trainX, trainY, batch_size=6),
	steps_per_epoch=len(trainX) // 6,
	validation_data=valAug.flow(testX, testY),
	validation_steps=len(testX) // 6,
	epochs=args["epochs"])

# evaluate the network
print("[INFO] evaluating network...")
y_score = model.predict(testX, batch_size=6)
